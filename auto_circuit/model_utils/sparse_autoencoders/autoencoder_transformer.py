...
def prune_weights_(
    model: HookedSAETransformer, srcs: Set[SrcNode]
) -> Tuple[HookedSAETransformer, Set[SrcNode]]:
    src_modules: Dict[t.nn.Module, List[SrcNode]] = defaultdict(list)
    [
        src_modules[
            module_by_name(model, ".".join(src.module_name.split(".")[:-1]))
        ].append(src)
        for src in srcs
    ]
    total_bytes_before = 0
    for module in src_modules.keys():
        for p in module.parameters():
            total_bytes_before += p.nbytes

    new_srcs = set()
    total_bytes = 0
    for module, srcs in src_modules.items():
        if isinstance(module, SAEWrapper):
            head_idxs = t.tensor([src.head_idx for src in srcs], device=module.device)
            for head_idx, src in enumerate(srcs):
                new_srcs.add(
                    SrcNode(
                        name=src.name,
                        module_name=src.module_name,
                        layer=src.layer,
                        head_idx=head_idx,  # set to None
            ...
            module.prune_latents(head_idxs)
        else:
    ...
    print(
        f"Parameter size before/after pruning: {total_bytes_before / 1024 ** 3:.2f} GB / {total_bytes / 1024 ** 3:.2f} GB"
    )
    return model, new_srcs
